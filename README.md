...
## 📊 Performance (v0.2.0 Benchmark)

Orbyは、データが重くなっても速度が変わらない**「予測可能なパフォーマンス」**を重視して設計されています。以下は1,000,000レコードを用いたベンチマーク結果です。

### 1. Dimension Scaling (1,000,000 Records)
次元数（Lane数）が増えても、Orbyの検索速度は極めて安定しています。
...
| 100万件 | Dim 4 | **7.99 ms** |
| 100万件 | Dim 8 | **8.78 ms** |
| 100万件 | Dim 12 | **10.37 ms** |

> 次元を12まで増やし、100万件の迷路を駆け抜けて、10ミリ秒

### 2. Vaultの「ステルス・オーバーヘッド」
フル永続化（Vaultモード）を使用しても、ディスク書き込みがスムーズです。

- **MemoryOnly**: 7.73 ms
- **Vault (Disk Backup)**: 7.77 ms
- **差分**: ** 0.04 ms (40µs)**