    ...
    T: AsRef<[u128]>,
{
    ...
    for item in items {
        ...
        let start = store.len * padded_dim;
...
/// Upsert ロジック: 既存 ID があれば Update、なければ Insert (末尾追加) を行います。
pub fn upsert(
    store: &mut OrbyStore,
    index: usize,
    id: u128,
...
pub fn purge_by_id(
    ...
    while i < store.len {
        ...
        if let Some(val) = store.buffer.get(start + index) {
            if val.as_u128() == id {
                let last_start = (store.len - 1) * padded_dim;

                // Swap
                if start != last_start {
                    for offset in 0..padded_dim {
                        store.buffer[start + offset] = store.buffer[last_start + offset];
                    }

                    if has_mirror {
                        // Mirror the swap: Write last_row to start_offset
                        let offset_bytes = crate::types::HEADER_SIZE + (start * 16) as u64;
                        let mut row_bytes = Vec::with_capacity(padded_dim * 16);
                        // Read from buffer because we just copied it
                        for offset in 0..padded_dim {
                            row_bytes.extend_from_slice(
                                &store.buffer[start + offset].as_u128().to_le_bytes(),
                            );
                        }
                        mirror_data.push((offset_bytes, row_bytes));
                    }
                ...
                // Remove (fill zero)
                store.buffer[last_start..last_start + padded_dim].fill(OrbitField::new(0));

                if has_mirror {
                    // Mirror zero fill
                    let offset_bytes = crate::types::HEADER_SIZE + (last_start * 16) as u64;
...
pub fn get_at(store: &OrbyStore, logical_index: usize) -> Option<Arc<[u128]>> {
    ...
    let physical_idx = store.len - 1 - logical_index;
    ...
    F: Fn(&[OrbitField]) -> bool + Sync + Send,
{
    ...
    let mut results: Vec<Arc<[u128]>> = store.buffer[..len * padded_dim]
